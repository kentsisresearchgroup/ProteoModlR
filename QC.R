...
QC <- function(data, mod="", skyline=T, MaxQuant=F){
  data$X=NULL
  ...
  if(skyline==T){
    ...
    if(ncol(data)!=10){
      stop(message("Data in the form of Skyline output should include 10 columns (even if empty) in the following order:Protein, Peptide, Modification, Gene.Name,Protein.Name, Condition, PatientID, Label, Run, Intensity. \n"))
    }
    colnames(data) <- c("Protein", "Peptide", "Modification", "Gene.Name",
    ...
    #Cast into MaxQuant format
    data$EntryID = paste(data$PatientID,data$Condition,data$Run, data$Label, sep = "_")
    data = data[,!(colnames(data) %in% c("PatientID","Run","Condition","Label"))]
    casted = dcast(data,Protein+Peptide+Modification+Gene.Name+Protein.Name~EntryID, value.var="Intensity")
  }
  ...
  if(MaxQuant==T){
    ...
    #Change column names 
    colnames(data[,1:5]) <-c("Protein", "Peptide", "Modification", "Gene.Name", "Protein.Name")
    if (sum(nchar(as.character(data$Protein))<6)>0){
      stop(message("Use standard protein names(e.g. Uniprot-ID). \n"))
    }
  ...
  data.mod<- data[grep(paste(modifications,collapse="|"),data$Modification, ignore.case = T),]
  ...
  data.mod.int <- ldply(1:length(data.mod$Protein), function(i){
      if(sum(data.mod[i,6:length(data.mod)]==0)<6){
      return(data.mod[i,])
    }
  }, .progress="text")
  ...
  classified.data <- ldply(unique(data.mod.int$Protein),function(i){
    temp<- subset(data.mod.int,Protein==i)
  ...
  data.melt <- melt(classified.data, id.vars=c("Protein", "Peptide", "Modification", "Abundance", "Stoichiometry", "Gene.Names", "Protein.Names" ))
  ...
  tracker=cbind(c("Raw Data", "After selecting for PTM of interest", 
                  ...
                  "After removing peptides with no intensity values",
