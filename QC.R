...
QC <- function(data, mod){
  ...
  data.mod <- ldply(unique(data.dup$Protein), function(i){
    ...
    if((sum(temp$Modification=="")==length(temp$Modification)|any(temp$Modification==mod))==TRUE){
      return(temp)
    }
  }, .progress="text")
...
#         }else{
          ...
          if(any(""==temp2$Modification)){
            temp2[which(temp2$Modification==""),"Modification"]="U"
            temp2[,"Stoichiometry"] = "Exact"
#             temp3 <- subset(temp2, Modification==mod)
